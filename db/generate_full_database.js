...
// Load all products
const allProducts = require('./all_products.json');
...
users.forEach(user => {
  ...
  for (let j = 0; j < numberOfPurchases; j++) {
    ...
    purchaseData.push({
      ...
      quantity: quantity,
      price: Math.floor(Math.random() * 100000) + 1000 // Random price between 1000-100000
    });
  }
});
...
const dbPath = path.join(__dirname, 'full_database.json');
